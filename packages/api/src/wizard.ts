...
<main>
  ...
  <section id="step-5" class="panel" hidden><h2>Pixel audit</h2><p class="lead">Compare the live witness vs your Next.js clone. Drag the slider to inspect differences.</p>
...
function mountCloneFrame(stage){const id=activeJob();const base='/v1/clones/'+id;$('clone-stage').textContent=stage||'';if(stage==='mirror')$('frame-clone').src=base+'/mirror-preview/?t='+Date.now();else $('frame-clone').src=base+'/app-preview/?t='+Date.now()}
async function buildLoop(){const id=activeJob();let seen=S.seen;const bar=$('stage-bar');bar.innerHTML=STAGES.map((s,i)=>'<span class="stage-chip" data-i="'+i+'">'+s+'</span>').join('');const poll=async()=>{try{const ev=await fetch('/v1/clones/'+id+'/events?after='+seen).then(r=>r.json());for(const e of ev.events||[]){seen++;const n=STAGE_EV[e.event];if(n!=null){bar.querySelectorAll('.stage-chip').forEach((c,i)=>{c.classList.toggle('done',i<n);c.classList.toggle('active',i===n)});$('build-pct').textContent=Math.min(99,Math.round((n+1)/STAGES.length*100))+'%'}if(e.event==='generated')mountCloneFrame('mirror');if(e.event==='app_build_done'&&e.ok)mountCloneFrame('export')}S.seen=seen;const wf=await fetch('/v1/clones/'+id+'/wip-files').then(r=>r.ok?r.json():null);if(wf?.files?.length)renderFiles(id,wf.files);const st=await fetch('/v1/clones/'+id).then(r=>r.json());if(st.status==='running')return;clearInterval(S.buildPoll);mountCloneFrame('export');$('build-pct').textContent='100%';bar.querySelectorAll('.stage-chip').forEach(c=>c.classList.add('done'));if(st.verify||st.status==='succeeded')setTimeout(()=>goAudit(),800)}catch{}};S.buildPoll=setInterval(poll,350);poll()}
function renderFiles(id,files){const tree=$('file-tree');const text=files.filter(f=>/\\.(tsx?|jsx?|css|json|html|md)$/.test(f)).slice(-80);tree.innerHTML=text.map(f=>'<div data-f="'+f+'">'+f+'</div>').join('');tree.querySelectorAll('div[data-f]').forEach(el=>{el.onclick=async()=>{tree.querySelectorAll('div').forEach(x=>x.classList.remove('active'));el.classList.add('active');const r=await fetch('/v1/clones/'+id+'/wip-files/'+encodeURIComponent(el.dataset.f));const d=await r.json();$('file-editor').textContent=d.content||'(binary)'}});if(!S.activeFile&&text.length){S.activeFile=text[text.length-1];tree.querySelector('[data-f="'+S.activeFile+'"]')?.click()}}
async function goAudit(){setStep(5);const id=activeJob();try{const a=await fetch('/v1/clones/'+id+'/audit').then(r=>r.json());const routes=S.scan?.routes?.filter(r=>S.routes.has(r.path))||[{path:'/',label:'Home'}];const sw=$('audit-routes');sw.innerHTML='';for(const rt of routes){const b=document.createElement('button');b.textContent=rt.label;b.onclick=()=>showDiff(a,1280,rt.path);if(rt.path==='/'||rt.path===S.scan?.entryPath){b.classList.add('active');showDiff(a,1280,rt.path)}sw.appendChild(b)}$('audit-meta').textContent=typeof a.score==='number'?'Score '+a.score.toFixed(1)+(a.worstDiffPct!=null?' · worst diff '+(a.worstDiffPct*100).toFixed(1)+'%':''):'Audit pending — validation may still be running'}catch(e){$('audit-meta').textContent='Audit: '+e}}
