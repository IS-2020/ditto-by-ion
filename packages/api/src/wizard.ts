...
.scan-banner{padding:10px 14px;border-radius:10px;background:#2a2418;border:1px solid #4a4020;color:var(--gold);font-size:13px;margin:0 0 16px}
.scan-banner a{color:var(--gold)}
...
<main>
  ...
  <section id="step-2" class="panel" hidden><h2>Scanning site…</h2><div id="scan-banner" class="scan-banner" hidden>Route scan unavailable on this host — cloning homepage only. <a href="https://github.com/IS-2020/ditto-by-ion#quick-start-local" target="_blank" rel="noopener">Run full API locally ↗</a></div><p class="lead" id="scan-status">Discovering routes and starting homepage clone.</p></section>
  <section id="step-3" class="panel" hidden><h2>Choose pages to replicate</h2><div id="scan-banner-3" class="scan-banner" hidden>Route scan unavailable on this host — cloning homepage only. <a href="https://github.com/IS-2020/ditto-by-ion#quick-start-local" target="_blank" rel="noopener">Run full API locally ↗</a></div><p class="lead">Homepage is already cloning. Check additional pages — they'll build in the background after the homepage finishes.</p>
...
function fallbackScan(url){let origin=url;try{origin=new URL(url).origin}catch{}return{url,origin,entryPath:'/',discovered:0,routes:[{path:'/',label:'Home',role:'entry',suggested:true}],suggestedPaths:['/'],scanUnavailable:true}}
function showScanBanner(){for(const id of['scan-banner','scan-banner-3']){const el=$(id);if(el)el.hidden=false}}
async function apiScan(url){const r=await fetch('/v1/scan',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({url,maxRoutes:20})});const d=await r.json();if(r.ok)return d;if(r.status===503||d.scanUnavailable)return fallbackScan(url);throw new Error(d.error||r.status)}
async function apiClone(url,opts){const r=await fetch('/v1/clones',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({url,options:opts})});const d=await r.json();if(!r.ok||d.error){if(r.status===503||d.cloneUnavailable)throw new Error('Clone API unavailable on this host. Set DITTO_WORKER_URL on Vercel or run locally: PORT=8899 npm run dev:api');throw new Error(d.error||d.details?.fieldErrors?.url?.[0]||r.status)}return d.jobId}
...
async function startHomeClone(){try{S.homeJob=await apiClone(S.url,{qualityTier:'production',mode:'single',preview:true,verify:true});saveProj();pollHomeStatus()}catch(e){$('home-status').textContent=String(e);$('wiz-pages-continue').disabled=true}}
...
$('wiz-start').onclick=async()=>{const url=norm($('wiz-url').value);if(!url)return;$('wiz-start').disabled=true;S.url=url;saveProj();setStep(2);startTimer();try{S.scan=await apiScan(url);if(S.scan.scanUnavailable){showScanBanner();$('scan-status').textContent='Route scan skipped — starting homepage clone.'}S.routes=top5(S.scan);renderRoutes();setStep(3);await startHomeClone()}catch(e){$('scan-status').textContent='Error: '+e;$('wiz-start').disabled=false}};
